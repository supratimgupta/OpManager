        ...
        public StatusDTO<UserMasterDTO> Update(UserMasterDTO data)
        {
            using (IDbSvc dbSvc = new DbSvc(_configSvc))
            {
                try
                {
                    ...
                    {
                        command.Parameters.Add("@UserName1", MySqlDbType.String).Value = data.UserName;
                        command.Parameters.Add("@Password1", MySqlDbType.String).Value = data.Password;
                    }
                    else
                    {
                        command.Parameters.Add("@UserName1", MySqlDbType.String).Value = DBNull.Value;
                        command.Parameters.Add("@Password1", MySqlDbType.String).Value = DBNull.Value;
                    }
                    //command.Parameters.Add("@RoleId", MySqlDbType.Int32).Value = data.Role.RoleId;
                    command.Parameters.Add("@LocationId", MySqlDbType.Int32).Value = data.Location.LocationId;
