        ...
        public DataTable GetAllRules()
        {
            using(IDbSvc dbSvc = new DbSvc(_configSvc))
            {
                try
                {
                    ...
                    command.CommandText = "SELECT TranRuleId, TranMasterId, StandardId, SectionId, UserMasterId, ClassTypeId, FirstDuedateAfterdays, DueDateincreasesBy, PenaltyCalculatedIn, PenaltyAmount, ActualAmount FROM dbo.TransactionRule WHERE Active=1";
